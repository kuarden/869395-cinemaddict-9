...
export class PageController {
  constructor(container, data) {
    ...
    this._statistics = new Statistics(data);
    this._subscriptions = [];
  ...
  init() {   
    ...
    if (this._data.length === 0) {
      ...
      render(this._container, this._statistics.element, `beforeend`);
  ...
  _showStatistics() {
    ...
    this._statistics.element.classList.remove(`visually-hidden`);
  }

  _hideStatistics() {
    this._filmList.element.classList.remove(`visually-hidden`);
    this._statistics.element.classList.add(`visually-hidden`);
  }
